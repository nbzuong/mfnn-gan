...
if __name__ == "__main__":
    ...
    paths1 = glob('datasets/t1_kj/', '*/0_*')  # center image만 가져옴
    # paths2 = glob('Datasets/images/t2_kj/', '*/0_*')  # center image만 가져옴
    
    # print(paths1)

    Apath=paths1
    
    BSET_folders = glob('outputs/HKPU_B_TA_CA_ver8/testoutput/','*_VALID')

    
    # print(BSET_folders)
    
    for idx,folder in enumerate(BSET_folders):
        
        epoch = re.sub(r'[^0-9]', '', folder.split('/')[-1])
        print(epoch)

        paths3 = glob(folder, '*/0_*')  # center image만 가져옴
        # paths4 = glob('Datasets/images/t2_enlighten_test/', '*/0_*')  # center image만 가져옴

        Bpath=paths3

        images1 = load_images(Apath,(180,70))
        images2 = load_images(Bpath,(180,70))
        
        fid_value = calculate_fid(images1, images2, options.use_multiprocessing, options.batch_size)
        print(fid_value)
        
        writecsv('HKPU_B_FID_valid.csv',[epoch, fid_value])